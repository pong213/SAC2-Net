...
def main(args):
    ...
    train_df = df[df["Dataset"] == args.train_dataset].reset_index(drop=True)
    val_df = df[df["Dataset"] == "smic"].reset_index(drop=True)
    ...
    class_counts = CLASS_COUNTS[args.train_dataset]
    ...
    for epoch in range(args.epochs):
        ...
        # Use test accuracy to select best epoch for this subject
        if val_acc > best_acc:
            best_acc = val_acc
            best_epoch = epoch
            best_preds = {
    ...
    print(
        f"Best epoch: {best_epoch} with Test Acc = {best_acc:.4f}"
    )

    # Build a result DataFrame for this subject based on best predictions
    result_df = val_df.copy()
    result_df["True_Label_Index"] = best_preds["true_indices"]
    result_df["Pred_Label_Index"] = best_preds["pred_indices"]
    result_df["Pred_Label_Name"] = [
        idx2label[i] for i in best_preds["pred_indices"]
    ]
    result_df["Pred_Confidence"] = best_preds["pred_probs"]
    result_df["Correct"] = (
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Cross-dataset benchmark training script (PyTorch)")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Train dataset name used for class-count configuration.",
    )
    parser.add_argument(
        "--output_excel",
        type=str,
        required=True,
        help="Path to save the Cross-Dataset prediction results Excel file.",
    )
