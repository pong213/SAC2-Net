...
This model is SAC2Net with CCF removed and is used for SASA pre-training using external datasets.
...
class SASAProjectionHead(nn.Module):
    ...
    Visual: GDConv pooling (spatial → vector) + linear projection
    ...
    def __init__(self, embed_dim: int = 512, projection_dim: int = 512, spatial_size: int = 7):
        ...
        self.mag_pool = nn.Sequential(
            nn.AdaptiveAvgPool2d(output_size=1),    # (B, D, 1, 1)
            nn.Flatten(1),  # (B, D)
        )
...
# =============================================================================
# MULTIMODAL MER MODEL
# =============================================================================

class SAC2Net_without_CCF(nn.Module):
    """
    ...
    def __init__(
        ...
        # --- Placeholder fusion for testing ---
        self.fusion_module = _PlaceholderFusion(embed_dim, spatial_size, num_classes)
        # --- End placeholder ---

        # =====================================================================
        # 5. Weight Initialization (EXCLUDING frozen modules)
        # =====================================================================
        self._init_weights()
    ...
    def forward(
        ...
        logits, fusion_aux = self.fusion_module(F_mag, F_flow)
    ...
    def get_trainable_param_groups(self, lr: float = 1e-4, lr_mult_encoder: float = 0.1):
        """
    ...
    def save_pretrained(self, save_directory: str, save_name: str = "multimodal_fer"):
        """
    ...
    def from_pretrained(
        ...
        Returns:
            Loaded SAC2Net_without_CCF instance
        """
    ...
    def load_checkpoint(
            ...
            optimizer: Optional[torch.optim.Optimizer] = None,
            device: Optional[torch.device] = None
    ) -> Tuple['SAC2Net_without_CCF', Dict]:
        """
...
# =============================================================================
# PLACEHOLDER FUSION (for testing without CCF)
# =============================================================================

class _PlaceholderFusion(nn.Module):
    """Minimal fusion placeholder that mimics CCF's interface."""

    def __init__(self, embed_dim, spatial_size, num_classes):
        super().__init__()
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.classifier = nn.Linear(embed_dim * 2, num_classes)

    def forward(self, F_mag, F_flow):
        f_mag = self.pool(F_mag).flatten(1)
        f_flow = self.pool(F_flow).flatten(1)
        logits = self.classifier(torch.cat([f_mag, f_flow], dim=-1))
        aux = {'reliability_mag': [], 'reliability_flow': []}
        return logits, aux
...
def example_usage():
    ...
    print("=" * 70)
    print("SAC2Net_without_CCF — Train/Test Mode Verification")
    print("=" * 70)
    ...
    # Create model
    model = SAC2Net_without_CCF(num_classes=5, embed_dim=512).to(device)
